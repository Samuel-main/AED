...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        if (IsKeyPressed(KEY_SPACE))
        {
            ...
            Bola *novoVetor = (Bola *)realloc(bolas, quantidadeBolas * sizeof(Bola));
            if (novoVetor != NULL) {
                bolas = novoVetor;
                ...
                nova->raio = (float)GetRandomValue(10, 15);
                nova->cor = (Color){
        ...
        BeginDrawing();
            ...
            DrawText(TextFormat("Bolas no Vetor Dinamico (RAM): %d", quantidadeBolas), 10, 10, 18, GREEN);
            DrawText("[ESPACO] Adiciona Bola | [BACKSPACE] Remove Bola", 10, ALTURA_JANELA - 25, 16, DARKGRAY);
