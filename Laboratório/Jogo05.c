...
void ordenarPorDistancia(void) {
    ...
    for (int i = 1; i < totalEntidades - 1; i++) {
        ...
        float dx1 = vetorEntidades[i]->pos.x - jogador->pos.x;
        float dy1 = vetorEntidades[i]->pos.y - jogador->pos.y;
        float menorDist = sqrtf(dx1 * dx1 + dy1 * dy1);
        ...
        if (menorIndice != i) {
            Entidade *temp = vetorEntidades[i];
            vetorEntidades[i] = vetorEntidades[menorIndice];
            vetorEntidades[menorIndice] = temp;
        }
    }
}
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        // Exercício 1: Reordena as entidades por distância do jogador
        ordenarPorDistancia();
